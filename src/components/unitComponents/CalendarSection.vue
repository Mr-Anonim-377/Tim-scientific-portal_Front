        ...
        <div class="calendar-container">
            ...
            <div class="calendar-years">
                ...
                <div
                    ...
                    :key="item.title"
                    class="calendar-years-dateWrapper"
                >
                    <!-- Vue элемент для анимации -->
                    <transition
                        :name="
                            left ? 'date-animation' : 'date-animation-reverse'
                        "
                    >
                        <!-- Элемент отображается только в случае, если индекс элемента в массиве соответствует this.index-->
                        <h2 v-if="tagsArray.indexOf(item) == index">
                            {{ eventByTags[`${item}`].dateTitle }}
                        </h2>
                    </transition>
                </div>
                <!-- Правая стрелка -->
            ...
            <div class="calendar-items">
                ...
                >
                    ...
                    >
                        <div
                            v-if="tagsArray.indexOf(item) == index"
                            class="calendar-item_container"
                        >
                            <CalendarItem
                                v-for="event in eventByTags[`${item}`].events"
                                :key="event.date"
                                :article-data="event"
                                :transform-mouth="transformMouth"
                            />
                        </div>
                    </transition>
                </div>
...
export default {
    ...
    methods: {
        ...
        arrowClick(direction) {
            if (direction == 'left') {
                this.index == 0
                    ? (this.index = this.tagsArray.length - 1)
                    : this.index--;
                this.left = false;
            } else {
                this.index == this.tagsArray.length - 1
